...
<template>
  <div v-if="!muted" :class="[$style.root, { [$style.children]: props.depth > 1 }]">
    <div :class="$style.main">
      <div v-if="note.channel" :class="$style.colorBar" :style="{ background: note.channel.color }" />
    ...
    <div v-else :class="$style.more">
      <MkA class="_link" :to="notePage(note)">
        {{ i18n.ts.continueThread }} <i class="ti ti-chevron-double-right" />
      </MkA>
    </div>
  </div>
...
watch([props.detail], async () => {
	await misskeyApi('notes/children', {
